function submitFunction() {
	try {
		...
		if(distance > 10800) {
			result = "The event hasn't started here yet.<div>&nbsp;</div>"
		}
		else if(distance >= 0) {
			...
			result = "Event ends in: " + ("0" + days).slice(-2) + "d " + ("0" + hours).slice(-2) + "h " + ("0" + minutes).slice(-2) + "m " + ("0" + seconds).slice(-2) + "s " + "<div>&nbsp;</div>";
		}
		else {
			result = "The event has already ended here.<div>&nbsp;</div>"
		}
		...
		document.getElementById("tzguessResult").innerHTML = "<div>&nbsp;</div>Please check your co-ordinates again.<div>&nbsp;</div>";
	}
}
